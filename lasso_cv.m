function [wopt,lambdaopt,RMSEval,RMSEest] = lasso_cv(t,X,lambdavec,K)
    ...
    randomind = reshape(randomind, N/K, K);
    randomind = sort(randomind);
    ...
    for kfold = 1:K
        ...
        valind = randomind(:, kfold); % Select validation indices
        estind = randomind(:, 1:K~=kfold); % Select estimation indices
        estind = estind(:);
        estind = sort(estind);
        ...
        for klam = 1:Nlam
            ...
            SEval(kfold,klam) = mean((t(valind) - X(valind,:)*what).^2); % Calculate validation error for this estimate
            SEest(kfold,klam) = mean((t(estind) - X(estind,:)*what).^2); % Calculate estimation error for this estimate
    ...
    lambdaopt = lambdavec(best_index); % Select optimal lambda 
