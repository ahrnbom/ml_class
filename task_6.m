...
[w, lambda, RMSE_val, RMSE_est] = multiframe_lasso_cv(Ttrain, Xaudio, lambdas, K);
...
function [Wopt,lambdaopt,RMSEval,RMSEest] = multiframe_lasso_cv(T,X,lambdavec,K)
    ...
    for kframe = 1:Nframes % First loop over frames
        ...
        for kfold = 1:K % Then loop over the folds
            ...
            for klam = 1:Nlam  % Finally loop over the lambda grid
                ...
                what = lasso_ccd(t_est, X_est, lambda, wold); % Calculate LASSO estimate at current frame, fold, and lambda

                SEval(kfold,klam) = SEval(kfold,klam) + sum((t_val - X_val*what).^2); % Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
                SEest(kfold,klam) = SEest(kfold,klam) + sum((t_est - X_est*what).^2); % Add estimation error at current frame, fold and lambda to the estimation error for this fold and lambda, summing tthe error over the frames
    ...
    [~, lambda_index] = min(MSEval);
    lambdaopt = lambdavec(lambda_index); % Assign optimal lambda 
