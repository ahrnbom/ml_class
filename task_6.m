...
[W, lambda, RMSEval, RMSEest] = multiframe_lasso_cv(Ttrain, Xaudio, lambdas, K);
...
Nval = floor(N/K);
cvhop = Nval;
randomind = randperm(Nval);% Select random indices for picking out validation and estimation indices. 
...
for kframe = 1:Nframes % First loop over frames
    ...
    for kfold = 1:K % Then loop over the folds
        ...
        for klam = 1:Nlam  % Finally loop over the lambda grid
            ...
            what = NaN% Calculate LASSO estimate at current frame, fold, and lambda
            
            SEval(kfold,klam) = NaN; % Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
            SEest(kfold,klam) = NaN; % Add estimation error at current frame, fold and lambda to the estimation error for this fold and lambda, summing tthe error over the frames
...
lambdaopt = NaN; % Assign optimal lambda 
